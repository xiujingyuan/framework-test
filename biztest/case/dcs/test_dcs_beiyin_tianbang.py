...
from biztest.util.easymock.rbiz.beiyin_tianbang import RepayBeiyinTianbangMock
from biztest.util.tools.tools import get_item_no, get_four_element, get_date_after
import common.global_const as gc


class TestDcsBeiyinTianbang(BaseRepayTest):
    env_test = gc.ENV
    environment = gc.ENVIRONMENT
    channel = "beiyin_tianbang"   #2023.3.22放款已删除该资方的流程
    period_count = 12
    ...
    @classmethod
    def setup_class(cls):
        cls.beiyin_mock = RepayBeiyinTianbangMock(rbiz_mock)
    ...
    def setup_method(self):
        ...
        self.item_no = 'bytb_' + get_item_no()
    ...
    @pytest.mark.DCS_test_bytb
    def test_bytb_channel_normal_repay(self):
        print("====================第一期正常还款（资方扣全额）====================")
        ...
        for order in order_list:
            paysvr_callback(order, 2 , channel_name="beiyin_tianbang_BAOFU_KUAINIU2")
        ...
        advanced_clearing.capital_settlement_notify(period_normal[0], period_normal[0], "normal",
                                                    "beiyin_tianbang_BEIYIN", "N", "", expect_operate_at)
        # step 3 执行task
    ...
    @pytest.mark.dcs_auto_test
    @pytest.mark.DCS_test_bytb
    def test_bytb_channel_overdue_repay_and_compensate(self):
        ...
        check_final = CheckDcsFinal(self.item_no, period_overdue[0], 'repay', self.channel + "_BAOFU_KUAINIU2")
        check_final.check_final_all()
        check_final.check_final_amount(capital)
        check_trans = CheckDcsTrans(self.item_no, period_overdue[0], 'repay', self.channel + "_BAOFU_KUAINIU2")
        ...
        run_dcs_task_by_count(self.item_no, 2)
        # step 4 检查数据
        check_trans.check_settlement_notify_clearing(period_overdue[0], period_overdue[0], "compensate", "")

    @pytest.mark.DCS_test_auto_test_demo
    @pytest.mark.dcs_auto_test
    @pytest.mark.DCS_test_bytb
    def test_bytb_compensate_and_repay_one_period(self):
    ...
    @pytest.mark.dcs_auto_test
    @pytest.mark.DCS_test_bytb
    def test_bytb_channel_payoff(self):
    ...
    @pytest.mark.dcs_auto_test
    @pytest.mark.DCS_test_bytb
    def test_bytb_channel_settle_our_fail_and_compensate(self):
        ...
        # paysvr_callback(order_capital, 2, "beiyin_tianbang_BEIYIN")
        # self.run_task_after_withhold_callback([order_capital])
        # 小单和我方代扣部分失败
        ...
        self.run_all_task_after_repay_success()
        self.run_task_after_withhold_callback([order_our, order_noloan])
        self.run_all_msg_after_repay_success()
        # step 7 检查biz的帐信息
        check_repay_biz(self.item_no)
        # 开始大单还款清分
