...
from biztest.interface.rbiz.rbiz_interface import simple_active_repay, fox_manual_withhold, sync_withhold_card, \
    bind_sms, asset_void_withhold, paysvr_callback, withhold_refund, run_refreshLateFeeV1_by_api, \
...
class TestRbizHaohanqianjingjing(BaseRepayTest):
    """
    Haohanqianjingjing repay
    """
    ...
    @pytest.mark.haohanqianjingjing
    ...
    @pytest.mark.rbiz_auto_test
    @pytest.mark.haohanqianjingjing
    def test_auto_normal_retry_not_balance(self):
        """
        自动代扣- 用新增的代扣卡来轮询
        """
        ...
        paysvr_callback(order_no, 3, channel_message="我什么也不相干")
    ...
    def test_refresh_late_fee_max(self):
        ...
        self.refresh_late_fee(self.item_num_no_loan)
        # 验证asset_tran罚息的金额
        check_asset_tran_by_item_no_and_type_and_period(self.item_no, "lateinterest", 1, asset_tran_amount=32576)
        check_asset_tran_by_item_no_and_type_and_period(self.item_num_no_loan, "lateinterest", 1,
                                                        asset_tran_amount=5148)
        # 验证sendmsg中消息的生成和发送情况

    @pytest.mark.rbiz_auto_test
    @pytest.mark.rbiz_refund
    def test_asset_void(self):
        asset_void_params = {
            "card_num_encrypt": self.four_element['data']["bank_code_encrypt"],
            "mobile_encrypt": self.four_element['data']["phone_number_encrypt"],
            "id_num_encrypt": self.four_element['data']["id_number_encrypt"],
            "username_encrypt": self.four_element['data']["user_name_encrypt"],
            "ref_no": self.item_no,
            "amount": self.pricipal_amount
        }
        resp, req_body = asset_void_withhold(gc.ENV, **asset_void_params)
        assert resp['content']['code'] == 2, f"资产取消接口调用失败,req_body={req_body},resp={resp}"
        self.run_all_task_after_repay_success()
        withhold_prov = get_withhold_by_item_no(self.item_no)
        self.task.run_task(withhold_prov[1]['withhold_channel_key'], 'withhold_order_sync')

    @pytest.mark.rbiz_auto_test
    @pytest.mark.rbiz_refund
    def test_repeat_repay_refund(self):
        # 退未还款的那笔
        asset_tran_amount = get_asset_tran_balance_amount_by_item_no_and_period(self.item_no, 1)
        asset_tran_amount_no_loan = get_asset_tran_balance_amount_by_item_no_and_period(self.item_num_no_loan, 1)
        params_combo_active = {
        ...
        merchant_key_no_loan = resp_combo_active['content']["data"]["project_list"][0]["order_no"]
        merchant_key = resp_combo_active['content']["data"]["project_list"][1]["order_no"]
        # 回调失败
        paysvr_callback(merchant_key_no_loan, 3)
        paysvr_callback(merchant_key, 3)
        wait_expect_task_appear(self.item_no, 'withhold_callback_process')
        self.task.run_task(self.item_num_no_loan, 'withhold_callback_process')
        self.task.run_task(self.item_no, 'withhold_callback_process')
        resp_combo_active_sec, req_body_sec = simple_active_repay(self.item_no, **params_combo_active)
        assert resp_combo_active_sec['content'][
                   'code'] == 0, f"主动合并代扣失败,req_body={req_body},resp_combo_active={resp_combo_active}"

        merchant_key_new_no_loan = resp_combo_active_sec['content']["data"]["project_list"][0]["order_no"]
        merchant_key_new = resp_combo_active_sec['content']["data"]["project_list"][1]["order_no"]
        paysvr_callback(merchant_key_new_no_loan, 3)
        paysvr_callback(merchant_key_new, 3)
        wait_expect_task_appear(self.item_no, 'withhold_callback_process')
        self.task.run_task(self.item_num_no_loan, 'withhold_callback_process')
        self.task.run_task(self.item_no, 'withhold_callback_process')
        # 回调成功
        paysvr_callback(merchant_key, 2)
        wait_expect_task_appear(self.item_no, 'withhold_callback_process')
        self.task.run_task(self.item_no, 'withhold_callback_process')

        paysvr_callback(merchant_key_new, 2)
        wait_expect_task_appear(self.item_no, 'withhold_callback_process')
        self.task.run_task(self.item_no, 'withhold_callback_process')
        self.task.run_task(merchant_key, 'assetWithholdOrderRecharge')
        self.task.run_task(merchant_key_new, 'assetWithholdOrderRecharge')

        # 退款
        resp_refund = withhold_refund(merchant_key_new, asset_tran_amount["asset_tran_balance_amount"])
        # 修改退款和退款查询接口
        self.paysvr_mock.update_withhold_refund(resp_refund['content']["data"]["refund_no"],
                                                asset_tran_amount["asset_tran_balance_amount"])
        self.paysvr_mock.update_withhold_refund_query(asset_tran_amount["asset_tran_balance_amount"])
        wait_expect_task_appear(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key, 'withhold_order_sync')
        self.task.run_task(merchant_key_new, 'withhold_order_sync')
        self.task.run_task(merchant_key_new_no_loan, 'withhold_order_sync')
        self.task.run_task(merchant_key_no_loan, 'withhold_order_sync')

    @pytest.mark.rbiz_auto_test
    @pytest.mark.rbiz_refund
    def test_repeat_repay_reverse_refund(self):
        # 退未还款的那笔
        asset_tran_amount = get_asset_tran_balance_amount_by_item_no_and_period(self.item_no, 1)
        asset_tran_amount_no_loan = get_asset_tran_balance_amount_by_item_no_and_period(self.item_num_no_loan, 1)
        params_combo_active = {
        ...
        resp_refund = withhold_refund(merchant_key, asset_tran_amount["asset_tran_balance_amount"], [merchant_key_new],
                                      item_no_list)
        assert resp_refund['content'][
                   'code'] == 2, f"退款失败,resp_refund={resp_refund}"
        # 修改退款和退款查询接口
        self.paysvr_mock.update_withhold_refund(resp_refund['content']["data"]["refund_no"],
                                                asset_tran_amount["asset_tran_balance_amount"])
        self.paysvr_mock.update_withhold_refund_query(asset_tran_amount["asset_tran_balance_amount"])
        wait_expect_task_appear(merchant_key, 'refund_online')
    ...
    # @pytest.mark.rbiz_auto_test
    @pytest.mark.rbiz_refund
    def test_repeat_repay_refund_withdraw(self):
        # 退未还款的那笔
        asset_tran_amount = get_asset_tran_balance_amount_by_item_no_and_period(self.item_no, 1)
        asset_tran_amount_no_loan = get_asset_tran_balance_amount_by_item_no_and_period(self.item_num_no_loan, 1)
        params_combo_active = {
        ...
        # 部分退款走代付
        resp_refund = withhold_refund(merchant_key_new, 1000)
        self.paysvr_mock.update_withdraw_query(1000, 2, receiver_name=self.four_element['data']["user_name_encrypt"],
                                               receiver_account=self.four_element['data']["bank_code_encrypt"],
                                               receiver_id=self.four_element['data']["id_number_encrypt"])
        self.task.run_task(merchant_key_new, 'refund_withdraw')
        self.task.run_task(merchant_key_new, 'refund_withdraw')
        self.task.run_task(merchant_key, 'withhold_order_sync')
        self.task.run_task(merchant_key_new, 'withhold_order_sync')
        self.task.run_task(merchant_key_new_no_loan, 'withhold_order_sync')
    ...
    @pytest.mark.rbiz_auto_test
    @pytest.mark.rbiz_refund
    def test_repeat_repay_refund_failed_withdraw(self):
        # 退未还款的那笔
        asset_tran_amount = get_asset_tran_balance_amount_by_item_no_and_period(self.item_no, 1)
        asset_tran_amount_no_loan = get_asset_tran_balance_amount_by_item_no_and_period(self.item_num_no_loan, 1)
        params_combo_active = {
        ...
        self.paysvr_mock.update_withhold_refund(resp_refund['content']["data"]["refund_no"],
                                                asset_tran_amount["asset_tran_balance_amount"])
        self.paysvr_mock.update_withhold_refund_query(asset_tran_amount["asset_tran_balance_amount"],pay_status="failed")
        wait_expect_task_appear(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key_new, 'refund_online')
        self.task.run_task(merchant_key, 'withhold_order_sync')
