...
from biztest.interface.rbiz.rbiz_interface import monitor_check, bind_sms, simple_active_repay, paysvr_callback, \
    asset_buyback
from biztest.util.easymock.rbiz.zhongbang_zhongji import RepayZhongbangZhongjiMock
...
@pytest.mark.rbiz_zhongbang_zhongji
class TestRbizZhongbangZhongji(BaseRepayTest):
    """
    zhongbang_zhongji 还款

    """
    loan_channel = "zhongbang_zhongji"
    our_sign_company = "tq,tqa,tqb"
    grant_principal = 500000

    @classmethod
    def setup_class(cls):
        monitor_check()
        cls.zbzj_mock = RepayZhongbangZhongjiMock(rbiz_mock)
    ...
    def setup_method(self):
        ...
        self.item_no, self.item_num_no_loan = asset_import_and_loan_to_success(self.loan_channel, self.four_element,
                                                                               item_no="zb" + get_item_no(),
                                                                               asset_amount=5000)

    def test_zhongbang_zhongji_active_advance_repay(self):
        """
        提前还1期 不允许
        """
        # step 1 主动还款 提前还款
    ...
    def test_zhongbang_zhongji_active_normal_repay(self):
        """
        正常还1期，(该资金方宽限期2天，到期2天都可以还款)
        资方扣1单，小单1单
        """
        # step 1 主动还款 到期日
        ...
        asset_tran_amount = get_asset_tran_balance_amount_by_item_no_and_period(self.item_no, 1)
        asset_tran_amount_no_loan = get_asset_tran_balance_amount_by_item_no_and_period(self.item_num_no_loan, 1)
        self.zbzj_mock.zhongbang_zhongji_normal_repay()
        ...
        self.zbzj_mock.zhongbang_zhongji_repay_query(msg='success', repay_status=1, repay_amt=436.79)
        ...
        self.wait_and_run_central_task(self.item_no, "ZhongbangZhongjiCapitalPush", excepts={"code": 0})
        self.wait_and_run_central_task(self.item_no, "GenerateCapitalNotify")
        # # 检查推送请求的参数
        check_capital_notify(self.item_no, withhold[0]["withhold_serial_no"], capital_notify_type="normal",
                             capital_notify_status='success')

    def test_zhongbang_zhongji_active_advance_settle_payoff(self):
        """
        在第一期内提前结清，此处写死资金方占用1天
        资方通道扣：大单当期本+当期部分费（算法：实际占用天数/当期总天数 x 当期费用总金额(四舍五入保留2位小数) ）+当期部分的息（必须小于等于当期利息）+剩余期次的所有本金
        我方扣：当期剩余的息+当期剩余的费+剩余期次的息和费
        """
        self.change_asset_due_at(-1, 1)  # 如果非大月份，该case可能跑不过，因为公式中的天数不一样了
        self.zbzj_mock.zhongbang_zhongji_repay_trial()
        # step 1 主动还款
        ...
        # 代扣金额
        # 资方代扣金额 = 大单当期本+当期费+当期部分的息（必须小于等于当期利息）+剩余期次的所有本金
        ...
        one_fee02 = get_asset_tran_by_item_no_and_type_and_period(self.item_no, "reserve", 1)[0][
            'asset_tran_amount']
        capital_withhold_amount = self.grant_principal + capital_fee + 3000  # 资金方扣的金额,mock第一期的代扣利息为30元，费用3673，写死
        ...
        self.zbzj_mock.zhongbang_zhongji_repay_advance_payoff()
        self.zbzj_mock.zhongbang_zhongji_repay_query(msg='success', repay_status=1, repay_amt=5030.00)
    ...
    def test_zhongbang_zhongji_active_overdue_repay(self):
        """
        宽限期外还款（逾期还款）
        推送消息给dcs
        """
        # step 1 主动还款 逾期3天，真正逾期1天
        ...
        self.wait_and_run_central_task(self.item_no, "GenerateCapitalNotify")
        check_capital_notify(self.item_no, capital_notify_type="overdue", capital_notify_status='success')

    def test_zhongbang_zhongji_protocol_sign_success(self):
        """
        众邦中际要走协议共享接口，此处走提前结清场景，需要在paysvr接口返回baofoo通道并且状态=1时会调用接口/withAgreeShar
        """
        self.change_asset_due_at(-1, 1)  # 如果非大月份，该case可能跑不过，因为公式中的天数不一样了
        # 提前修改需要使用到的接口mock返回
        self.zbzj_mock.zhongbang_zhongji_repay_trial()
        self.paysvr_mock.update_query_protocol_channels_not_bind_sms()
        self.zbzj_mock.zhongbang_zhongji_withagreeshar()
        ...
        card_bind = {
            ...
            "card_bind_channel": "zhongbang_zhongji_sign",
