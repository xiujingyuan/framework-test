...
class TestRbizHamitianshanTianbang(BaseRepayTest):
    ...
    """
    loan_channel = "hami_tianshan_tianbang"
    ...
    @pytest.mark.hami
    def test_active_advance_repay_suc(self):
        """
        提前还1期
        """
        # step 1 修改资产到期日
    ...
    def test_auto_normal_repay_fail_retry_switch_to_paysvr(self):
        ...
        paysvr_callback(order_no, 3)
        order_no = get_withhold_order_by_item_no(self.item_no)[1]["withhold_order_serial_no"]
        paysvr_callback(order_no, 3)
        ...
        self.task.run_task(self.item_no, 'auto_withhold_execute')
        self.task.run_task_by_order_no_count(self.item_no, 3)

        withhold_ready = get_withhold_by_item_no(self.item_no)
        order_no = withhold_ready[0]["withhold_serial_no"]
        paysvr_callback(order_no, 2)
        self.task.run_task_by_order_no(self.item_no)
        self.task.run_task_by_order_no_count(order_no, 3)
        self.task.run_task_by_order_no(self.item_no)
    ...
    # # @pytest.mark.rbiz_auto_test
    # @pytest.mark.hami
    def test_fox_overdue_four_day_settle_payoff(self):
        """
        逾期第4天提前结清，砍单，只扣第1期
        """
        ...
        # step 4 逾期数据检查，大单1单
        withhold = get_withhold_by_item_no(self.item_no)
        self.task.run_task_by_order_no_count(withhold[0]["withhold_request_no"], count=8)
        self.task.run_task_by_order_no_count(withhold[0]["withhold_serial_no"], count=3)
        # 代扣顺序： 第1期逾期部分我方扣
        check_withhold_order_by_order_no(withhold[0]["withhold_serial_no"], "baidu_tq3_quick")
