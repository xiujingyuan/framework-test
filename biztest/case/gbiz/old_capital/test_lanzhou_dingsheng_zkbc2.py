...
# from biztest.util.easymock.zhongke_lanzhou import ZhongkeLanzhouMock
from biztest.util.easymock.gbiz.lanzhou_dingsheng_zkbc2 import LanZhouDingShengZkbcMock
...
class TestLanzhouDingshengZkbc2(BaseTestCapital):
    """
       gbiz_lanzhou_dingsheng_zkbc2
       author: zhimengxue
       date: 20210826
       """

    def init(self):
        super(TestLanzhouDingshengZkbc2, self).init()
        self.channel = "lanzhou_dingsheng_zkbc2"
        self.capital_mock = LanZhouDingShengZkbcMock(gbiz_mock)
        update_gbiz_capital_lanzhou_dingsheng_zkbc2()
        update_all_channel_amount()
    ...
    def register(self, item_no, four_element):
        ...
        sms_seq = capital_regiest(self.channel, four_element, item_no, from_system='strawberry',
                                  action_type='GetSmsVerifyCode', way='lanzhou_dingsheng_zkbc2', step_type='PROTOCOL', seq='')['data']['actions'][0]['extra_data']['seq']
        capital_regiest(self.channel, four_element, item_no, from_system='strawberry',
                        action_type='CheckSmsVerifyCode', way='lanzhou_dingsheng_zkbc2', step_type='PROTOCOL', seq=sms_seq)
        capital_regiest_query(self.channel, four_element, item_no, from_system='strawberry')
    ...
    @pytest.mark.gbiz_lanzhou_dingsheng_zkbc2
    @pytest.mark.gbiz_zlanzhou_dingsheng_zkbc2_loan_success
    @pytest.mark.parametrize("app, source_type, count",
                             [
                                 #("草莓", "apr36", 12),
                                 ("草莓", "irr36_quanyi", 12),
                             ])
    def test_zlanzhou_dingsheng_zkbc2_loan_success(self, case, app, source_type, count):
        """
        放款成功
        """
    ...
    # @pytest.mark.gbiz_auto_test
    @pytest.mark.gbiz_zlanzhou_dingsheng_zkbc2
    @pytest.mark.parametrize("count", [12])
    def test_lanzhou_dingsheng_zkbc2_canloan_fail(self, case, count):
        """
        canloan失败切换资金方
        :param case:
        :param count:
        ...
        check_wait_change_capital_data(item_no, 4, "lanzhou_dingsheng_zkbc2->校验资金量失败;")

    # @pytest.mark.gbiz_auto_test
    @pytest.mark.gbiz_lanzhou_dingsheng_zkbc2
    @pytest.mark.parametrize("count", [12])
    def test_lanzhou_dingsheng_zkbc2_customer_fail(self, case, count):
        """
        客户信息推送失败切资方
        """
        four_element = get_four_element()
    ...
    @pytest.mark.gbiz_lanzhou_dingsheng_zkbc2
    @pytest.mark.parametrize("count", [12])
    def test_lanzhou_dingsheng_zkbc2_face_fail(self, case, count):
        """
        人脸识别失败切资方
        """
        four_element = get_four_element()
    ...
    # @pytest.mark.gbiz_auto_test
    @pytest.mark.gbiz_lanzhou_dingsheng_zkbc2
    @pytest.mark.test_lanzhou_dingsheng_zkbc2_loan_fail
    @pytest.mark.parametrize("count", [12])
    def test_lanzhou_dingsheng_zkbc2_loan_fail(self, case, count):
        """
        放款支用失败切资方
        """
        four_element = get_four_element()
    ...
    def test_register_send_sms_fail_01(self, case):
        ...
        resp = capital_regiest(self.channel, four_element, item_no, way=self.channel, step_type='PROTOCOL', action_type='GetSmsVerifyCode')
        # 断言：步骤状态为2，发短信环节失败2
        Assert.assert_equal(0, resp['code'])
    ...
    def test_register_check_sms_fail_01(self, case):
        ...
        resp = capital_regiest(self.channel, four_element, item_no, way=self.channel, step_type='PROTOCOL', action_type='CheckSmsVerifyCode', seq=sms_seq)
        Assert.assert_equal(0, resp['code'])
        # 断言：步骤状态为2，验证环节失败2
        check_data(resp['data'], status=2, way=self.channel, step_type="PROTOCOL", interaction_type="SMS")
    ...
    def test_register_success_01_normal(self, case):
        ...
        check_capital_account_data(item_no, four_element, self.channel, self.channel, account_status=4, step_status=0, user_key=sms_seq)
        # 开户查询
        resp = capital_regiest_query(self.channel, four_element, item_no)
        Assert.assert_equal(0, resp['code'])
        # 断言：账户、步骤、环节状态都为0-成功
        ...
        check_capital_account_data(item_no, four_element,  self.channel, self.channel, account_status=0, step_status=0, user_key=sms_seq)
    ...
    def test_register_success_02_without_seq(self, case):
        ...
        self.capital_mock.update_tied_card()
        self.capital_mock.update_tied_card_query_success()
        resp = capital_regiest(self.channel, four_element, item_no, way=self.channel, step_type='PROTOCOL', action_type='CheckSmsVerifyCode')
        Assert.assert_equal(0, resp['code'])
        # 断言：步骤和环节状态为0-成功
        check_data(resp['data'], status=0, way=self.channel, step_type="PROTOCOL", interaction_type="SMS")
        check_data(resp['data']['actions'][0], action_type='GetSmsVerifyCode', status=0)
