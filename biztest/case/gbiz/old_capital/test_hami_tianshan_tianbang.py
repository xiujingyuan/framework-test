...
from biztest.function.gbiz.gbiz_db_function import update_router_cp_amount_all_to_zero, \
    update_router_capital_plan_amount, \
    update_all_channel_amount, update_task_by_item_no_task_type
...
# @pytest.mark.gbiz_hami_tianshan_tianbang
class TestHamiTianshanTianbang(BaseTestCapital):
    """
       gbiz_hami_tianshan_tianbang
       author: zhimengxue
       date: 20201224
       """

    def init(self):
        super(TestHamiTianshanTianbang, self).init()
        self.channel_hami_tianbang = "hami_tianshan_tianbang"
        self.hami_mock = HamiTianShanMock(gbiz_mock)
        self.payment_mock = PaymentMock(gbiz_mock)
        update_gbiz_capital_hami_tianshan_tianbang()
    ...
    def register(self, item_no, four_element):
        self.payment_register(self.payment_mock, self.channel_hami_tianbang, four_element, item_no)
    ...
    def loan_to_success(self, item_no):
        ...
        prepare_attachment(self.channel_hami_tianbang, item_no)
    ...
    @pytest.mark.gbiz_hami_tianshan_tianbang_loan_success
    ...
    def test_hami_tianshan_tianbang_loan_success(self, case, app, source_type, count):
        """
        放款成功
        :param case:
        :param count:
        ...
        item_no, asset_info = asset_import(self.channel_hami_tianbang, four_element, count, 8000, app, source_type, '',
                                           '110000')

        self.register(item_no, four_element)
        self.loan_to_success(item_no)
        check_asset_tran_data(item_no)
        check_request_log_by_channel(item_no, self.channel_hami_tianbang)
        # 小单部分
    ...
    def test_hami_tianshan_tianbang_canloan_fail(self, case, count):
        """
        canloan失败切换资金方
        :param case:
        :param count:
        ...
        """
        four_element = get_four_element(id_num_begin="6522")
        item_no, asset_info = asset_import(self.channel_hami_tianbang, four_element, count, 8000, "香蕉", "apr36", '',
                                           '110000')
        ...
        update_router_capital_plan_amount(0, today, self.channel_hami_tianbang)
        self.task.run_task(item_no, "ApplyCanLoan", excepts={"code": 0})
        # canloan执行之后恢复
        update_router_capital_plan_amount(3333333333, today, self.channel_hami_tianbang)
        check_wait_change_capital_data(item_no, 4, "hami_tianshan_tianbang->校验资金量失败;", "AssetCanLoanFailedEvent")

    @pytest.mark.parametrize("count", [6])
    def test_hami_tianshan_tianbang_apply_fail(self, case, count):
        """
        进件查询失败切换资金方
        :param case:
        :param count:
    ...
    @pytest.mark.parametrize("count", [12])
    def test_hami_tianshan_tianbang_apply_fail_assetvoid_rollback(self, case, count):
        """
    ...
    @pytest.mark.parametrize("count", [6])
    def test_hami_tianshan_tianbang_loan_fail(self, case, count):
        """
        放款失败切资方
        :param case:
        :param count:
    ...
    def test_hami_tianshan_tianbang_certificate(self, case):
        item_no = fake_asset_data(self.channel_hami_tianbang, status="payoff")
        resp = certificate_apply(item_no)
        Assert.assert_equal(0, resp["code"])
        check_asset_event_exist(item_no, self.channel_hami_tianbang, "CERTIFICATE_GENERATE_EVENT")
        self.hami_mock.update_postapply_success()
