...
from biztest.util.easymock.gbiz.mozhi_jinmeixin import MozhiJinmeixinMock
...
# @pytest.mark.gbiz_mozhi_jinmeixin
class TestMozhiJinmeixin(BaseTestCapital):
    """
          gbiz_mozhi_jinmeixin
          author: zhimengxue
          date: 20210811
          """

    def init(self):
        super(TestMozhiJinmeixin, self).init()
        self.channel = "mozhi_jinmeixin"
        self.capital_mock = MozhiJinmeixinMock(gbiz_mock)
        self.payment_mock = PaymentMock(gbiz_mock)
        update_gbiz_capital_mozhi_jinmeixin()
        update_gbiz_capital_mozhi_jinmeixin_const()
    ...
    @pytest.mark.gbiz_mozhi_jinmeixin_loan_success
    ...
    def test_mozhi_jinmeixin_loan_success(self, case, app, source_type, period):
        """
        放款成功
        """
    ...
    def test_mozhi_jinmeixin_canloan_fail(self, case, count):
        """
        canloan失败切换资金方
        :param case:
        :param count:
        ...
        item_no, asset_info = asset_import(self.channel, four_element, count, 7000, "香蕉", "irr36_quanyi", '',
                                           '110000')
        self.register(item_no, four_element)
        self.task.run_task(item_no, "AssetImport", excepts={"code": 0})
        self.msg.run_msg(item_no, "AssetImportSync")
        ...
        check_wait_change_capital_data(item_no, 4, "mozhi_jinmeixin->校验资金量失败;", "AssetCanLoanFailedEvent")

    @pytest.mark.parametrize("count", [6])
    def test_mozhi_jinmeixin_loan_fail(self, case, count):
        """
        放款失败切资方
        :param case:
        :param count:
        ...
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 5000, "香蕉", "irr36", '',
                                           '110000')
    ...
    @pytest.mark.parametrize("count", [12])
    def test_mozhi_jinmeixin_apply_fail(self, case, count):
        """
        老用户授信额度不足进件时切换资金方
        :param case:
        :param count:
        ...
        """
        self.four_element = get_four_element()
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 6000, "香蕉", "irr36_quanyi", '',
                                           '110000')
        self.register(item_no, self.four_element)
    ...
    @pytest.mark.parametrize("count", [6])
    def test_mozhi_jinmeixin_apply_query_fail(self, case, count):
        """
        新用户授信失败切换资金方
        :param case:
        :param count:
        ...
        """
        self.four_element = get_four_element()
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 3000, "香蕉", "irr36_quanyi", '',
                                           '110000')
        self.register(item_no, self.four_element)
    ...
    @pytest.mark.parametrize("count", [12])
    def test_mozhi_jinmeixin_applycomfirm_fail(self, case, count):
        """
        ...
        """
        self.four_element = get_four_element()
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 7000, "香蕉", "irr36", '',
                                           '110000')
        self.register(item_no, self.four_element)
    ...
    @pytest.mark.parametrize("count", [6])
    def test_mozhi_jinmeixin_apply_refuse_fail(self, case, count):
        """
        曾经授信拒绝的用户，进件失败切换资金方
        :param case:
        :param count:
        ...
        """
        self.four_element = get_four_element()
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 2000, "香蕉", "irr36", '',
                                           '110000')
        self.register(item_no, self.four_element)
    ...
    @pytest.mark.parametrize("count", [12])
    def test_mozhi_jinmeixin_apply_success(self, case, count):
        """
        ...
        """
        self.four_element = get_four_element()
        item_no, asset_info = asset_import(self.channel, self.four_element, count, 6000, "香蕉", "irr36", '',
                                           '110000')
        self.register(item_no, self.four_element)
