...
from biztest.config.gbiz.gbiz_lanzhou_haoyue_chongtian_config import *
...
import pytest
from biztest.util.easymock.gbiz.lanzhou_haoyue_chongtian import LanzhouHaoyueChongtianMock
from biztest.util.tools.tools import get_four_element


@pytest.mark.gbiz_auto_test
@pytest.mark.gbiz_lanzhou_haoyue_chongtian
class TestLanzhouHaoyueChongtian(BaseTestCapital):
    """
       gbiz_lanzhou_haoyue_chongtian
       author: zhimengxue
       date: 20221206
       """
    def init(self):
        super(TestLanzhouHaoyueChongtian, self).init()
        self.channel = "lanzhou_haoyue_chongtian"
        self.capital_mock = LanzhouHaoyueChongtianMock(gbiz_mock)
        update_gbiz_capital_lanzhou_haoyue_chongtian()
        update_gbiz_capital_lanzhou_haoyue_chongtian_const()
    ...
    def register(self, four_element, item_no):
        self.capital_mock.update_bindCard_check()
    ...
    def loan_to_success(self, item_no):
        ...
        self.capital_mock.update_loanapply_trial(item_no)
    ...
    @pytest.mark.gbiz_lanzhou_haoyue_chongtian_loan_success
    @pytest.mark.parametrize("app, source_type, period",
                             [("香蕉", "apr36", 12),
                              # ("草莓", "irr36", 12),
                              ])
    def test_lanzhou_haoyue_chongtian_loan_success(self, case, app, source_type, period):
        """
        放款成功
        """
    ...
    def test_lanzhou_haoyue_chongtian_canloan_fail(self, case, app, source_type, period):
        """
        canloan失败切资方
        :param app:
        :param source_type:
        ...
        check_wait_change_capital_data(item_no, 4, "lanzhou_haoyue_chongtian->校验资金量失败;", "AssetCanLoanFailedEvent")

    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_apply_fail(self, case, app, source_type, period):
        """
        LoanApplyNew 失败切资方
        :param app:
        :param source_type:
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_applyquery_fail(self, case, app, source_type, period):
        """
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_creditapply_fail(self, case, app, source_type, period):
        """
        ...
        self.capital_mock.update_user_infoPushQuery()
        self.task.run_task(item_no, "LoanApplyQuery", excepts={"code": 0})
        self.capital_mock.update_loancreditapply(code='999999', msg='mock_试算失败')
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_confirm_fail(self, case, app, source_type, period):
        """
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_creditquery_fail(self, case, app, source_type, period):
        """
        ...
        self.task.run_task(item_no, "LoanCreditQuery", excepts={"code": 0})
        check_wait_change_capital_data(item_no, 3000000, "成功-LOAN_FAILURE-\\(拒绝代码：null， 拒绝原因：mock_放款失败\\)", "LoanCreditFailedEvent")
        check_wait_blacklistcollect_data(item_no, asset_info, 3000000, "成功-LOAN_FAILURE-\\(拒绝代码：null， 拒绝原因：mock_放款失败\\)", "id_card")

    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_lanzhou_haoyue_chongtian_confirmquery_fail(self, case, app, source_type, period):
        """
        LoanCreditQuery 失败切资方
        :param app:
        :param source_type:
