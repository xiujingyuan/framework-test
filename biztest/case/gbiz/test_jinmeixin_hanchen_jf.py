...
from biztest.config.gbiz.gbiz_jinmeixin_hanchen_jf_config import *
...
import pytest
from biztest.util.easymock.gbiz.jinmeixin_hanchen_jf import JinmeixinHanchenJfMock
from biztest.util.tools.tools import get_four_element


@pytest.mark.gbiz_auto_test
@pytest.mark.gbiz_jinmeixin_hanchen_jf
class TestJinmeixinHanchenJf(BaseTestCapital):
    """
       gbiz_jinmeixin_hanchen_jf
       author: zhimengxue
       date: 20221024
       """
    def init(self):
        super(TestJinmeixinHanchenJf, self).init()
        self.channel = "jinmeixin_hanchen_jf"
        self.capital_mock = JinmeixinHanchenJfMock(gbiz_mock)
        update_gbiz_capital_jinmeixin_hanchen_jf()
        update_gbiz_capital_jinmeixin_hanchen_jf_const()
    ...
    def loan_to_success(self, item_no):
        ...
        check_asset_event_exist(item_no, self.channel, "JMX_HC_JF_LOAN_PRE_SUCCESS")
        ...
        check_asset_event_exist(item_no, self.channel, "JMX_HC_JF_LOAN_POST_SUCCESS")
    ...
    @pytest.mark.gbiz_jinmeixin_hanchen_jf_loan_success
    @pytest.mark.parametrize("app, source_type, period",
                             [("香蕉", "apr36", 12),
                              # ("草莓", "irr36", 12),
                              ])
    def test_jinmeixin_hanchen_jf_loan_success(self, case, app, source_type, period):
        """
        放款成功
        """
    ...
    def test_jinmeixin_hanchen_jf_canloan_fail(self, case, app, source_type, period):
        """
        canloan失败切资方
        :param app:
        :param source_type:
        ...
        check_wait_change_capital_data(item_no, 4, "jinmeixin_hanchen_jf->校验资金量失败;", "AssetCanLoanFailedEvent")

    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_apply_fail(self, case, app, source_type, period):
        """
        LoanApplyNew 失败切资方
        :param app:
        :param source_type:
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_apply_fail_01(self, case, app, source_type, period):
        """
        LoanApplyNew 失败切资方，授信额度不足
        """
        four_element = get_four_element(id_num_begin="53")
        item_no, asset_info = asset_import(self.channel, four_element, period, 6000, app, source_type)
        self.register(item_no,four_element)
        ...
        prepare_attachment(self.channel, item_no)
        self.task.run_task(item_no, "LoanPreApply", excepts={"code": 0})
        self.capital_mock.update_quota_query_old_user(availableQuota=10, endDate='20500528')
        self.task.run_task(item_no, "LoanApplyNew", excepts={"code": 0})
        self.capital_mock.update_creditresult_query(startDate='20221021', endDate='20291231', quota=2000000,
                                                    creditStatus='03',  message='mock请求失败')
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_apply_fail_02(self, case, app, source_type, period):
        """
        LoanApplyNew 失败切资方，授信日期过期
        """
        four_element = get_four_element(id_num_begin="53")
        item_no, asset_info = asset_import(self.channel, four_element, period, 6000, app, source_type)
        self.register(item_no,four_element)
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_applyquery_fail_01(self, case, app, source_type, period):
        """
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_applyquery_fail_02(self, case, app, source_type, period):
        """
        授信额度不足，失败切换资金方
        :param app:
        :param source_type:
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_applyquery_fail_03(self, case, app, source_type, period):
        """
        授信已过期，失败切换资金方
        :param app:
        :param source_type:
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_confirm_fail(self, case, app, source_type, period):
        """
    ...
    @pytest.mark.parametrize("app, source_type, period", [("香蕉", "apr36", 12)])
    def test_jinmeixin_hanchen_jf_confirmquery_fail(self, case, app, source_type, period):
        """
