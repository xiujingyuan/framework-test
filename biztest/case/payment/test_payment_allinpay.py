...
class TestPaymentAllinpay:
    env = gc.ENV
    env_test = get_env_dict(env)
    db_test_payment = gc.PAYMENT_DB

    @pytest.mark.payment
    @pytest.mark.payment_allinpay
    def test_allinpay_protocol(self):
        """
        通联协议支付测试
        :return:
        """
        self.four_element = get_four_element_in_payment()
        channel_name = "allinpay_my_protocol"  #allinpay_tq_protocol测试环境不可用了，证书被删除
        # 组装获取验证码参数
    ...
    @pytest.mark.payment
    @pytest.mark.payment_allinpay
    def test_allinpay_withhold(self):
        """
        通联协议支付测试
        :return:
        """
        self.four_element = get_four_element_in_payment()
        channel_name = "allinpay_tq_withhold"
        params_card_num = self.four_element["bank_code_encrypt"]
    ...
    # @pytest.mark.payment_allinpay
    # def test_allinpay_withdraw(self):  #测试环境晓艳已经把通道删了
    #     """
    #     通联代付测试
    #     :return:
    #     """
    #     self.four_element = get_four_element_in_payment()
    #     channel_name = "qsq_allinpay_my_protocol"
    #
    #     # 发起代付
