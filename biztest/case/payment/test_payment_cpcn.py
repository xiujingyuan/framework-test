...
@pytest.mark.payment
@pytest.mark.payment_cpcn
class TestPaymentCpcn:
    env = gc.ENV
    env_test = get_env_dict(env)
    db_test_payment = gc.PAYMENT_DB

    def test_cpcn_quick(self):
        """
        中金快捷支付测试
        :return:
        """
        self.four_element = get_four_element_in_payment("建设银行", bank_code_suffix='10')
        channel_name = "cpcn_tq_quick"
        # 组装获取验证码参数
        resp_bindsms = withhold_bindsms(self.four_element, channel_name)
        assert resp_bindsms["code"] == 0, '获取验证码成功'

        # 传入通道进行绑卡
        resp_withhold_bind = withhold_bind(self.four_element, channel_name, 134679, resp_bindsms["data"]["verify_seq"])
    ...
    def test_cpcn_withhold(self):
        """
        通联协议支付测试
        :return:
        """
        self.four_element = get_four_element_in_payment()
        channel_name = "cpcn_tq_withhold"
        params_card_num = self.four_element["bank_code_encrypt"]
