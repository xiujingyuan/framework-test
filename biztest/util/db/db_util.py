...
class DataBase(object):
    ...
    def connect(self, env):
        if env in DataBase.pools.keys():
            ...
            if self.environment == "dev":
                ...
                fd = open("./resource/database.config")
            elif self.environment == "prod":
                fd = open("./resource/database_prod.config")
    ...
    def close_connects(cls):
        if DataBase is not None:
            for db, pool in DataBase.pools.items():
                try:
                    LogUtil.log_info("关闭数据库：%s" % db)
                    pool.connection().close()
                    pool.close()
                    LogUtil.log_info("关闭数据库成功")
                except:
                    pass
    ...
    def __del__(self):
        if hasattr(self, 'close_connects'):
            self.close_connects()


class PyMysql:

    def __init__(self, env, environment, country="china", **kwargs):
        ...
        if self.country == "china":
            ...
            dbserver_name = "global_" + env + "_india"
        elif self.country == "indonesia":
            dbserver_name = "yinni_global_" + env
        elif self.country == "thailand":
            dbserver_name = "global_" + env + "_thailand"
        elif self.country == "philippines":
            dbserver_name = "global_" + env + "_philippines"
        elif self.country == "mexico":
            dbserver_name = "global_" + env + "_mexico"
        else:
        ...
        for config in configs['databases']:
            if config["dbserver"] == dbserver_name:
                ...
                LogUtil.log_debug("conect to %s with port %s by PyMysql" % (env, self.port))
                # LogUtil.log_debug(pprint(inspect.stack()[:5]))
                break
    ...
    def close_conn(self):
        try:
            if hasattr(self, 'cur') and self.cur:
                self.cur.close()
            if hasattr(self, 'py_mysql') and self.py_mysql:
