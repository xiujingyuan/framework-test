...
class MozhiJinmeixinMock(Easymock):
    def update_access_success(self):
        api = "/mozhi/jinmeixin/user.access"
        mode = '''{
                  "code": 0,
                  "message": "成功",
                  "data": {
    ...
    def update_accountquery_nodata(self):
        api = "/mozhi/jinmeixin/bind.bankcard.list"
        mode = '''{
                    "code": 0,
                    "message": "成功",
                    "data": [ ]
    ...
    def update_get_sms_code_success(self):
        api = "/mozhi/jinmeixin/bind.bankcard"
        mode = '''{
                  "code": 0,
                  "message": "外层测试msg",
                  "data": {
    ...
    def update_verify_sms_success(self):
        api = "/mozhi/jinmeixin/bind.sms.verify"
        mode = '''{
                  "code": 0,//0成功，其他失败
    ...
    def update_user_credit_info_noactive(self):
        api = "/mozhi/jinmeixin/user.credit.info"
        mode = '''{
                    "code": 0,
                    "message": "成功",
                    "data": {
    ...
    def update_user_credit_apply(self):
        api = "/mozhi/jinmeixin/limit.apply"
        mode = '''{
                  "code": 0, //0-成功，  其他重试
                  "message": "测试进件",
                  "data": {
    ...
    def update_user_credit_apply_result_success(self):
        api = "/mozhi/jinmeixin/credit.apply.result"
        mode = '''{
                  "code": 0,
                  "message": "成功",
                  "data": {
    ...
    def update_user_loan_apply_success(self):
        api = "/mozhi/jinmeixin/loan.apply"
        mode = '''{
                      "code": 0, //0-成功，其他重试
                      "message": "成功",
                      "data": {
    ...
    def update_user_loan_result_success(self, asset_info):
        api = "/mozhi/jinmeixin/loan.result"
    ...
    def update_repayPlanquery_success(self, asset_info):
        api = "/mozhi/jinmeixin/loan.record.detail"
        ...
        for i in range(asset_info['data']['asset']['period_count']):
            ...
            repayment_plan['termInterest'] = float(fee_info['interest'])
            # 资方返回担保费，我们透传
            repayment_plan['termGuaranteeAmount'] = float(fee_info['reserve']) + float(fee_info['consult'])
            repayment_plan['termAmount'] = repayment_plan['termPrincipal'] + repayment_plan['termInterest'] + \
                                           repayment_plan['termGuaranteeAmount']
    ...
    def update_contractpush_ftp_success(self):
        api = "/capital/ftp/upload/mozhi_jinmeixin"
        mode = '''{
                  "code": 0,
                  "message": "",
                  "data": {
